...
2. Install .NET SDK and runtime to the Image (**As of April 2022**: The RTO C# examples supports .NET Core *version 2.1*).
3. Install additionally packages for VS Code and the Remote - Containers extension.
...
```
Then, we set this ```.env.devcontainer``` file to Docker on runtime with the devcontainer.json's ```runArgs``` configuration:
...
Once the devcontainer creation succeeded, developers can access RTO credentials via the following methods:
* In a container's bash: via the ```$<variable name>``` syntax like ```$RTO_USERNAME```
* In VS Code context: via the ```${env:<variable name>}``` syntax like ```${env:RTO_USERNAME}```
